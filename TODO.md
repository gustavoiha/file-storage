...
- [ ] Handle duplicate files that are already stored, allowing users to skip duplicate uploads when they choose to.
...
- [x] Add a "Purge now" action in Trash so each file can be purged immediately after confirmation, bypassing trash retention time. Plan: `/Users/gustavoiha/Personal/file-storage/plans/purge-now-feature-plan.md`.
